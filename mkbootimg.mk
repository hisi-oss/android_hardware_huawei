...
$(INSTALLED_BOOTRAMDISKIMAGE_TARGET): $(MKBOOTIMG) $(AVBTOOL) $(BUILT_RAMDISK_TARGET)
	...
	$(hide) $(call assert-max-image-size,$@,$(BOARD_BOOT_RAMDISKIMAGE_PARTITION_SIZE))
	$(hide) $(AVBTOOL) add_hash_footer \
	  --image $@ \
	  --partition_size $(BOARD_BOOT_RAMDISKIMAGE_PARTITION_SIZE) \
	  --partition_name boot $(INTERNAL_AVB_BOOT_SIGNING_ARGS) \
	  $(BOARD_AVB_BOOT_ADD_HASH_FOOTER_ARGS)
...
$(INSTALLED_RECVENDORIMAGE_TARGET): $(MKBOOTIMG) $(AVBTOOL) $(INSTALLED_RECVENDOR_RAMDISK_TARGET)
	...
	$(hide) $(call assert-max-image-size,$@,$(call get-hash-image-max-size,$(BOARD_RECOVERY_VENDORIMAGE_PARTITION_SIZE)))
	$(hide) $(AVBTOOL) add_hash_footer \
	  --image $@ \
	  --partition_size $(BOARD_RECOVERY_VENDORIMAGE_PARTITION_SIZE) \
	  --partition_name recovery $(INTERNAL_AVB_RECOVERY_SIGNING_ARGS) \
	  $(BOARD_AVB_RECOVERY_ADD_HASH_FOOTER_ARGS)
